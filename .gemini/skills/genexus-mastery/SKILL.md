...
| `genexus_analyze` | One tool covers `linter`, `navigation`, `hierarchy`, `impact`, `data_context`, `ui_context`, `pattern_metadata`, and `summary`. `explain` remains only as a compatibility envelope and returns `NotImplemented`; use `summary`, `context`, or `genexus_read` instead. |
