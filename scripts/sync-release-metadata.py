...
def catalog_data(root: Path) -> tuple[str, list[dict[str, Any]], Path]:
    ...
    return primary, normalized, path
...
def generated_block(primary: str, entries: list[dict[str, Any]]) -> str:
    names = display_names(entries)
    supported = ", ".join(names)
    return "\n".join(
        [
            f"<!-- BEGIN GENERATED: {MARKER} -->",
            f"Supported SDK majors: **{supported}**.",
...
def render_generated_doc(version: str, primary: str, entries: list[dict[str, Any]]) -> str:
    rows = [
...
def expected_files(root: Path, version: str) -> dict[Path, str]:
    primary, entries, catalog_path = catalog_data(root)
    primary_path = str(primary_entry(primary, entries, catalog_path)["defaultInstallPath"])
    names = display_names(entries)
    supported = ", ".join(names)
    description = (
        f"Read, edit, and analyze {supported} KB objects from Claude, Cursor, and AI agents."
    )
    package = read_json(root / "package.json")
    package["description"] = (
        f"{supported} MCP server — read, edit, and analyze Knowledge Base objects directly "
    ...
    return {
        ...
        root / "README.md": replace_marked_block(
            (root / "README.md").read_text(encoding="utf-8"), generated_block(primary, entries), "README.md"
        ),
        root / "AGENTS.md": replace_marked_block(
            (root / "AGENTS.md").read_text(encoding="utf-8"), generated_block(primary, entries), "AGENTS.md"
        ),
        root / "docs" / "generated" / "supported-versions.md": render_generated_doc(version, primary, entries),
    }
