...
| `genexus_wwp` | `list`, `settings_templates`, `settings_read` | `add_action`, `update_action`, `move_action`, `remove_action`, `add_tab`, `move_tab`, `remove_tab`, `set_table_type`, `add_grid_attribute`, `replace_web_component_with_user_action`, `settings_edit` |
...
| `genexus_wwp` | active | Typed WorkWithPlus grid actions, tabs, nested controls, native table-type changes, grid attributes, and native WebComponent-to-DropDownComponent replacement with preview, concurrency checks, verification, and rollback |
